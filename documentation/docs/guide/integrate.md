...
Described below is an example key and how to use this key in your web services.

### Generate Key
The example key below is generated by AuthC on first start (or uses an existing keyfile if one is already available). AuthC uses this key to SIGN JWTs that are made available for your applications. 

Both the key on the AuthC server and your web service must match, even if they are hosted on different servers. This requires that your web service references the key produced by AuthC either through an environment variable or file.
...
Next, import the JWK into your web service. One way to do this is by reading a file (make a copy of the file generated by AuthC).
